...
namespace FilebotApi
{
    public static class FileBotLogParser
    {
        internal static bool TryParse(string _line, out FileBotResult _result)
        {
            ...
            var keys = Actions.Keys.Where(_line.StartsWith).ToList();
        ...
        public static RenameResult Rename(string _line, DateTime _dateTime)
        {
            ...
            var split = trimmed.Split(new[] {"] to ["}, StringSplitOptions.None);
            Debug.Assert(split.Length == 2);

            var source = split[0].TrimStart('[');
            var destination = split[1].TrimEnd(']');

            var result = new RenameResult(Filebot.ActionType.TEST, source, destination, _line, _dateTime);
            return result;
        }
        ...
        public static SkipResult Skip(string _line, DateTime _dateTime)
        {
            ...
            var split = trimmed.Split(new[] {"] because ["}, StringSplitOptions.None);
            Debug.Assert(split.Length == 2);

            var source = split[0].TrimStart('[');
            //var destination = split[1].TrimEnd(']');

            var result = new SkipResult(source, _line, _dateTime);
            return result;
        }

        public static FileBotResult Log(string _line, DateTime _dateTime)
        {
            var message = $"[LOG] {_line}";
            return new FileBotResult(message, _dateTime);
        }

        private static Dictionary<string, Func<string, DateTime, FileBotResult>> Actions { get; } =
            new Dictionary<string, Func<string, DateTime, FileBotResult>>
            {
                {TEST_RENAME, Rename},
                {MOVE_RENAME, Rename},
